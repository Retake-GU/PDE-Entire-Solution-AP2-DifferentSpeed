...
for step = 1:Nt
    ...
    if mod(step, plot_interval) == 0
        ...
        minus_z_all = [minus_z_all; -z(:)];
        t_all = [t_all; t * ones(Nz, 1)]; % 列向量
        v_all = [v_all; v(:)];          % 列向量
        flip_v = flip(v(:)); 
        flip_v_all = [v_all; flip(v(:))];          % 列向量
    end
end
...
minus_z_vals = linspace(min(minus_z_all), max(minus_z_all), 200);

t_vals = linspace(min(t_all), max(t_all), 200);

[Z, T] = meshgrid(z_vals, t_vals);
[minus_Z, T] = meshgrid(minus_z_vals, t_vals);

% 插值（确保输入是列向量）
V = griddata(z_all, t_all, v_all, Z, T, 'cubic');
minus_V = griddata(minus_z_all, t_all, flip_v_all, minus_Z, T, 'cubic');

% 绘图
figure;
hold on;
% surf(Z, T, V, 'EdgeColor', 'none');
surf(minus_Z, T, V, 'EdgeColor', 'none');
